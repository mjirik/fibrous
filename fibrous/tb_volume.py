...
class TBVolume(tree.TubeSkeletonBuilder):
    ...
    def _add_cylinder(self, p1m, p2m, rad, id):
        ...
        p1 = [
            p1m[0] / self.voxelsize_mm[0],
            p1m[1] / self.voxelsize_mm[1],
            p1m[2] / self.voxelsize_mm[2],
        ]
        p2 = [
            p2m[0] / self.voxelsize_mm[0],
            p2m[1] / self.voxelsize_mm[1],
            p2m[2] / self.voxelsize_mm[2],
        ]
        logger.log(
            UNDERDEBUG, "p1_px: " + str(p1[0]) + " " + str(p1[1]) + " " + str(p1[2])
        )
        logger.log(
            UNDERDEBUG, "p2_px: " + str(p2[0]) + " " + str(p2[1]) + " " + str(p2[2])
        )
        ...
        for i in range(0, len(xvalues)):
            ...
            try:
                ...
                if no_index_error_occured:
                    ...
                    traceback.print_exc()
                    logger.warning(
                        "Cylinder drawing out of bounds. Other same type warnings are suppressed."
                    )
        ...
        # a lot faster)
        cut_up = max(0, round(min(p1[0], p2[0]) - (rad / min(self.voxelsize_mm)) - 2))
        # ta 2 je kuli tomu abyh omylem nurizl
        cut_down = min(
            self.shape[0], round(max(p1[0], p2[0]) + (rad / min(self.voxelsize_mm)) + 2)
        )
        cut_yu = max(0, round(min(p1[1], p2[1]) - (rad / min(self.voxelsize_mm)) - 2))
        cut_yd = min(
            self.shape[1], round(max(p1[1], p2[1]) + (rad / min(self.voxelsize_mm)) + 2)
        )
        cut_xl = max(0, round(min(p1[2], p2[2]) - (rad / min(self.voxelsize_mm)) - 2))
        cut_xr = min(
            self.shape[2], round(max(p1[2], p2[2]) + (rad / min(self.voxelsize_mm)) + 2)
        )
        logger.log(
            UNDERDEBUG,
            "cutter_px: z_up-"
    ...
    def finish(self):
        ...
        """
        progress_step = 1.0 / (
            len(self.intensity_profile) * len(self._cylinders_params)
        )
        progress = 0.0
