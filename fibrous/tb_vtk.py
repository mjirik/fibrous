...
def get_sphere(
    center, radius, resolution=10, start_phi=None, end_phi=None, axis=0, **kwargs
):
    sph = get_sphere_source(
        center, radius, resolution, start_phi, end_phi, axis, **kwargs
    )
    # sph.Update()
    return sph
...
def polygon_radius_compensation_factos(
    ...
    if polygon_radius_selection_method == "inscribed":
        ...
        # from .. import geometry3d as g3
        radius_compensation_factor = regular_polygon_perimeter_equivalent_radius(
            cylinder_resolution
        )
        ...
        # from .. import geometry3d as g3
        radius_compensation_factor = regular_polygon_area_equivalent_radius(
            cylinder_resolution
        )
        cylinder_radius_compensation_factor = radius_compensation_factor
        sphere_radius_compensation_factor = radius_compensation_factor
        ...
        cylinder_radius_compensation_factor *= regular_polygon_area_equivalent_radius(
            cylinder_resolution
        )
        cylinder_radius_compensation_factor_long = cylinder_radius_compensation_factor

        spl1 = InterpolatedUnivariateSpline(x_cvse, y_cvse)
        sphere_radius_compensation_factor *= 1.0 / spl1(cylinder_resolution)
        sphere_radius_compensation_factor_long = sphere_radius_compensation_factor

    elif polygon_radius_selection_method == "cylinder surface + sphere error":
        # analytically compensated cylinder + sphere compensate by measurement
        cylinder_radius_compensation_factor = regular_polygon_perimeter_equivalent_radius(
            cylinder_resolution
        )
        cylinder_radius_compensation_factor_long = cylinder_radius_compensation_factor
        ...
        sphere_radius_compensation_factor_long = (
            sphere_radius_compensation_factor / spl2(cylinder_resolution)
        )

    elif (
        polygon_radius_selection_method == "cylinder volume + sphere error + join error"
    ):
        # sphere like objects
        # analytically compensated cylinder + sphere compensate by measurement
        cylinder_radius_compensation_factor = regular_polygon_area_equivalent_radius(
        ...
        logger.error(
            "Unknown compensation method '{}'".format(polygon_radius_selection_method)
        )
        print(
            "Unknown compensation method '{}'".format(polygon_radius_selection_method)
        )
...
def gen_tree(
    ...
    cylinder_radius_compensation_factor, sphere_radius_compensation_factor, cylinder_radius_compensation_factor_long, sphere_radius_compensation_factor_long = (
        factors
    )
    ...
    for br in tree_data_old:
        ...
        if length == 0:
            tube = get_sphere(
                uv, radius * sphere_radius_compensation_factor, sphere_resolution
            )
        else:
            tube = get_tube(
                radius,
...
def get_tube_old(
    ...
    if length > 0:
        cylinder = get_cylinder(
            point, length, cylinder_radius, direction, resolution=cylinder_resolution
        )
...
def vt_file_2_vtk_file(
    infile, outfile, text_label=None, tube_shape=True, use_simple_cylinder_method=False
):
    """
    ...
    vt2vtk_file(
        ...
        text_label,
        tube_shape=tube_shape,
        use_simple_cylinder_method=use_simple_cylinder_method,
    )
...
def main():
    ...
    parser.add_argument(
        "outputfile", default="output.vtk", nargs="?", help="output file"
    )
    parser.add_argument(
        "-l",
        "--label",
